...
(define (add-bound name vars) 
    (let ((bounds (vars->bounds vars)) (frees (vars->frees vars)))
        (if (null? bounds)
            (cons (list (cons name (varid name))) frees)
            (cons (cons (cons name (varid name)) bounds) frees))))
(define (add-free name vars) 
    (let ((bounds (vars->bounds vars)) (frees (vars->frees vars)))
        (if (null? frees)
            (cons bounds (list (cons name (varid name))))
            (cons bounds (cons (cons name (varid name)) frees)))))
...
(define (mark-scope x vars)
    (let* ((names (cadr x)) (body (caddr x)) (nvars (fold-right add-bound vars names)))
        (replace-end (mark body nvars) (mark (inst->next x) vars))))

(define (mark-define x vars)
    (let* ((name (cadr x)) (nvars (add-bound name vars)))
        (mark (inst->next x) nvars)))

(define (mark-close x vars)
    (let* ((names (cadr x)) (body (caddr x)) (nvars (fold-right add-bound (vars->bounds-as-frees vars) names)))
